...
{
	return (f32)(platform->GetPerformanceCounter() - core->starting_ticks) /
	       (f32)platform->GetPerformanceFrequency();
}
...
internal void TimerPause(Timer *timer)
{
	...
	timer->paused = true;
	timer->paused_ticks =
		platform->GetPerformanceCounter() - timer->start_ticks;
...
internal void TimerResume(Timer *timer)
{
	...
	timer->paused = false;
	timer->start_ticks =
		platform->GetPerformanceCounter() - timer->paused_ticks;
...
internal f32 TimerGetElapsedSecondsF(Timer *timer)
{
	if (timer->started) {
		if (timer->paused) {
			return (f32)timer->paused_ticks /
			       (f32)platform->GetPerformanceFrequency();
		}

		return (f32)(platform->GetPerformanceFrequency() -
			     timer->start_ticks) /
		       (f32)platform->GetPerformanceFrequency();
	}
