...
Mat4 Mat4::inverse() const
{
	...
	Vec4 vec0 = { m01, m00, m00, m00 };
	Vec4 vec1 = { m11, m10, m10, m10 };
	Vec4 vec2 = { m21, m20, m20, m20 };
	Vec4 vec3 = { m31, m30, m30, m30 };
	...
	for (int i = 0; i < 4; i++) {
		inverse.v[0][i] = inv0.v[i] * sign_a[i];
		inverse.v[1][i] = inv1.v[i] * sign_b[i];
		inverse.v[2][i] = inv2.v[i] * sign_a[i];
		inverse.v[3][i] = inv3.v[i] * sign_b[i];
	}

	Vec4 row0 = { inverse.m00, inverse.m01, inverse.m02, inverse.m03 };
	Vec4 m0 = { m00, m10, m20, m30 };
	Vec4 dot0 = m0 * row0;
