...
internal void
RenderGraphExecuteRenderPasses(RenderGraph *graph, RenderContext *render_context)
{
	...
	for(i32 i = 0; i < graph->pass_count; i++)
	{
		...
		switch(pass.type)
		{
			case RenderPassType_Graphics:
			{
				...
				pass.graphics.Record(render_context, &render_info, pass.context);
			}
			break;
...
internal void
RenderGraphPush(RenderGraph *graph, RenderPass *pass)
{
	Assert(graph->pass_count < ArraySize(graph->passes) && "Cannot push more render passes.");
