...
static R_PASS_RECORD_DEF(R_SkyboxPassFn)
{
	...
	args.frame_data_buffer = G_DeviceBufferAddress       (device, user_data->frame_data_buffer);
	args.vertex_buffer     = G_DeviceBufferAddress       (device, user_data->skybox_mesh->vertex_buffer);
	args.cubemap_texture   = G_DeviceTextureViewBindless (device, user_data->cubemap);
