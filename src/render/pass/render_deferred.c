...
// Reference: https://songho.ca/opengl/gl_sphere.html
...
R_DeferredCreateLightSphereMesh(R_DeferredRenderer *dr)
{
	...
	f32 sector_step = 2.f * PI_F32 / (f32)sector_count;
	f32 stack_step  =       PI_F32 / (f32)stack_count;
	...
	{
		f32 theta = PI_F32 * 0.5f - (f32)i * stack_step;
						 ...
						 const R_DrawStream *draw_stream)
{
	...
	R_TextureInfo depth_info = R_TextureInfoInit();
	depth_info.format = GFX_DeviceDepthFormat(dr->device);
	...
	GFX_ShaderKey shader = AST_Get(dr->assets, dr->model_shader, AST_Type_Shader)->shader.key;
...
R_PASS_RECORD_DEF(R_DeferredLightingPassFn)
{
	...
	// -- Ambient lighting (fullscreen triangle).
	{
		...
		GFX_CmdDraw(cmd, 3, 1, 0, 0);
	}

	// -- Direct point lighting (light sphere volumes, additive blending).
	{
		...
		for (u32 i = 0; i < light_count; i++)
		{
			...
			GFX_CmdDrawIndexed(cmd, data->light_sphere_mesh->index_count, 1, 0, 0, i);
		}
	}
}
						 ...
						 GFX_TextureKey brdf)
{
	...
	GFX_ShaderKey ambient_shader = AST_Get(dr->assets, dr->ambient_lighting_shader,      AST_Type_Shader)->shader.key;
	GFX_ShaderKey direct_shader  = AST_Get(dr->assets, dr->direct_lighting_point_shader, AST_Type_Shader)->shader.key;
