...
// NOTE(kp): This is not a complicated hash table implementation at all.
//           Just a table of linked lists.

internal void
HashTableInit(HashTable *table, MemoryArena *arena, u64 node_size)
{
	table->arena = arena;
...
internal void *
HashTableAddElement(HashTable *table, u64 hash, void *element)
{
...
internal void *
HashTableFetchElement(HashTable *table, u64 hash)
{
	u32 index = hash % ArraySize(table->buckets);
