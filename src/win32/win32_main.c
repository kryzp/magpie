...
internal void
Win32LoadCoreCode(Win32CoreCode *core_code, const char *source_dll)
{
	...
	if(core_code->handle)
	{
		core_code->CoreInit            = GetProcAddress(core_code->handle, "CoreInit");
		core_code->CoreUpdate          = GetProcAddress(core_code->handle, "CoreUpdate");
		core_code->CoreDestroy         = GetProcAddress(core_code->handle, "CoreDestroy");
		core_code->CoreBeforeHotReload = GetProcAddress(core_code->handle, "CoreBeforeHotReload");
		core_code->CoreAfterHotReload  = GetProcAddress(core_code->handle, "CoreAfterHotReload");
	}
}

internal void
Win32UnloadCoreCode(Win32CoreCode *core_code)
{
