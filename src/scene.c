...
SceneObjectFromHandle(Scene *scene, u32 handle)
{
...
internal void
SceneResolveAdding(Scene *scene)
{
...
internal void
SceneResolveRemoving(Scene *scene)
{
...
internal void
SceneRemoveSceneObject(Scene *scene, u32 handle)
{
	scene->pending_removal[scene->pending_removal_count++] = handle;
}

internal u32
SceneRegisterObject(Scene *scene,
					...
					SceneObjectFlags flags)
{
	...
	object->material_id = RenderContextUploadMaterial(render_context, assets, material);
