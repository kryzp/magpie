...
{
	SceneObject *render_object =
		SceneObjectFromHandle(scene, pass->direct_batches->object_id);
	...
	for (DirectBatch *curr = pass->direct_batches->next; curr;
		...
		if (are_same_mesh && are_same_material) {
			...
			IndirectBatch *new_batch =
				MemoryArenaPush(arena, sizeof(IndirectBatch));
...
internal MeshPass MeshPassInit(Scene *scene, MemoryArena *arena)
{
	MeshPass pass = { 0 };
	...
	// NOTE(kp): RENDER BATCHES.
	{
		for (i32 i = 0; i < scene->object_count; i++) {
			DirectBatch *direct_batch =
				MemoryArenaPush(arena, sizeof(DirectBatch));
	...
	{
		pass.batches =
			MeshPassCompactDrawsToBatches(&pass, scene, arena);
	}
