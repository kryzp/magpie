...
internal void LoadCoreCode(MacOSCoreCode *core_code)
{
	...
	if (core_code->handle) {
		...
		core_code->CoreDestroy =
			dlsym(core_code->handle, "CoreDestroy");
		core_code->CoreBeforeHotReload =
			dlsym(core_code->handle, "CoreBeforeHotReload");
		core_code->CoreAfterHotReload =
			dlsym(core_code->handle, "CoreAfterHotReload");
	}
}
...
i32 main(void)
{
	u32 init_flags = SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_JOYSTICK |
			 SDL_INIT_GAMEPAD | SDL_INIT_HAPTIC | SDL_INIT_EVENTS |
	...
	u64 window_flags = SDL_WINDOW_VULKAN | SDL_WINDOW_HIGH_PIXEL_DENSITY;

	window = SDL_CreateWindow(WINDOW_TITLE, DEFAULT_WINDOW_WIDTH,
				  DEFAULT_WINDOW_HEIGHT, window_flags);

	if (!window) {
		DebugLogCrash("Failed to create SDL window: %s",
			      SDL_GetError());
		return -1;
	}
	...
	while (!global_platform.exit) {
		...
		while (SDL_PollEvent(&ev)) {
			...
			case SDL_EVENT_KEY_DOWN: {
				...
				global_platform.kb_pressed[ev.key.scancode] =
					!prev_st.kb_down[ev.key.scancode];
			} break;

			case SDL_EVENT_KEY_UP: {
				global_platform.kb_down[ev.key.scancode] =
					false;
				global_platform.kb_released[ev.key.scancode] =
					prev_st.kb_down[ev.key.scancode];
			} break;

			case SDL_EVENT_MOUSE_BUTTON_DOWN: {
				global_platform.mb_down[ev.button.button] =
					true;
				global_platform.mb_pressed[ev.button.button] =
					!prev_st.mb_down[ev.button.button];
			} break;

			case SDL_EVENT_MOUSE_BUTTON_UP: {
				global_platform.mb_down[ev.button.button] =
					false;
				global_platform.mb_released[ev.button.button] =
					prev_st.mb_down[ev.button.button];
			} break;
			...
			case SDL_EVENT_MOUSE_MOTION: {
				...
				global_platform.mouse_position =
					v2(ev.motion.x, ev.motion.y);
				global_platform.mouse_delta =
					v2(ev.motion.xrel, ev.motion.yrel);
				global_platform.mouse_screen_position =
					v2(spx, spy);
			} break;

			case SDL_EVENT_MOUSE_WHEEL: {
				global_platform.mouse_wheel =
					v2(ev.wheel.x, ev.wheel.y);
			} break;
			}
		}
		...
		if (global_platform.window_width != prev_st.window_width ||
			...
			if (global_platform.window_width >= 0 &&
				...
				SDL_SetWindowSize(
					window, global_platform.window_width,
